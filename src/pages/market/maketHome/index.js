                ...
                <Spin spinning={loadingState} classs={theme ? 'market-loadingNull' : 'market-loading'}>
                    <table className="tabPane-content">
                        <thead>
                            <tr>
                                {/* 名称 */}
                                <th style={{ width: !theme ? '20%' : '33.3%' }}>{t('market.freeContractName')}</th>
                                {/* 最新价 */}
                                <th style={{ width: !theme ? '15%' : '33.3%' }}>{t('market.freeNewPrice')}</th>
                                {/* 涨幅 */}
                                <th style={{ width: !theme ? '15%' : '33.3%' }} onClick={ () => this.setFilter() }>{t('market.freeIncrease')}</th>
                                {
                                    !theme ?
                                        <React.Fragment>
                                            {/* 最高价 */}
                            ...
                            <React.Fragment>
                                {
                                    marketTable.length > 0 ? marketTable.map((item) => (
                                        <tr key={item.id} onClick={() => { 
                                            ...
                                            <td style={{ width: !theme ? '15%' : '33.3%' }}>{item.now || '--'}</td>
                                            <td className={this.filterUpDown1(item.dailyReturn, 1)} style={{ width: !theme ? '15%' : '33.3%' }}>
                                    ...
                                    :
                                    <div align="center" className="nodata">{t('market.nodata')}</div>
                                }
                            </React.Fragment>
                        </tbody>
                    </table>
