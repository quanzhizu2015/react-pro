...
@connect(state => ({
    ...
    websocketParams: state.websocketParams,
    clearWebsocketParams: state.clearWebsocketParams
}))
export default class Websocket extends Component {
    ...
    componentWillReceiveProps(props) {
        // console.log(props.websocketParams)
        if (this.isOnline === true && props.wsObj && props.wsObj.readyState !== 3) {
            props.dispatch(props.setWsStatus(true))
        }
        ...
        if (props.wsConnet && !this.props.wsConnet && props.websocketParams) {
            props.websocketParams.forEach((item) => {
                props.dispatch(props.sendWs(props.wsObj, JSON.stringify(item)))
            })
            props.dispatch(props.clearWebsocketParams())
        }
    }
