...
export default class TradeInput extends Component {
    ...
    add() {
        ...
        if (value) {
            ...
            const val = String(BigDecimal(value).add(BigDecimal(this.props.region)).floatValue())
    ...
    del() {
        ...
        if (value && Number(value) - Number(this.props.region) > 0) {
            const val = String(BigDecimal(value).subtract(BigDecimal(this.props.region)).floatValue())
            this.props.setValue(val)
        } else {
        ...
        return (
            <div className={cs('trade-main trade-input ft-trade-border', this.props.className)}>
                ...
                >
                    ...
                    {this.textInput()}
                    <span onClick={this.inputFocus} className="trade-input-ts ft-theme-label">{this.props.unit}</span>
                </div>
                {this.editInput()}
            </div>
        )
